...
metric_functions = {
    "weighted phonetic product": metrics.weighted_phonetic_product,
...
def sanitise(string):
    ...
    return "".join([c for c in string.strip().lower() if c not in banned_characters])
...
def format_transcriptions(transcriptions):
    ...
    if len(clean_transcriptions) == 0:
        ...
        return [
            (sanitise(pair.split(":")[0]), sanitise(pair.split(":")[1]))
...
def evolve():
    ...
    try:
        words, rules = engine.run_engine(
            words, generations, transcriptions, reverse, metric, optimisation_function
        )
...
def apply():
    ...
    try:
        evolved_words = engine.apply_rules(words, rules, transcriptions, reverse)
    except Exception as e:
        return jsonify({"error": str(e)})
