...
def print_duplicates(list_of_tuples):
    ...
    duplicate_strings = [
        string for string, count in Counter(feature_strings).items() if count > 1
    ]
        ...
        for diacritic in diacritics:
            ...
            if base_segment.meets_conditions(diacritic["conditions"]):
                ...
                feature_strings.append(
                    (IPA_representation, deparse.feature_string(diacritic_segment))
                )

                if base_segment.meets_conditions(
                    {"positive": ["syllabic"]}
                ) and not diacritic_segment.meets_conditions({"positive": ["long"]}):
                    long_segment = (
                        base_segment
                        + Segment(
                            diacritic["applies"].get("positive", []),
                    ...
                    feature_strings.append(
                        (IPA_representation + "ː", deparse.feature_string(long_segment))
                    )
